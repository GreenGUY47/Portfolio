      ...
      <div className="relative z-10 max-w-screen-xl 2xl:max-w-screen-2xl mx-auto w-full px-5 sm:px-8 lg:px-14 xl:px-20 2xl:px-28 py-14 xl:py-20 2xl:py-28 flex flex-col flex-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12 xl:mb-16 2xl:mb-20">
          <div>
            ...
            <h1 className="font-black cursor-pointer text-white leading-none tracking-tighter" style={{ fontSize: "clamp(2.8rem, 7vw, 7rem)" }}>
              {t('cv.heading')}
            </h1>
          </div>
          ...
          <div className="lg:col-span-2 cursor-pointer flex flex-col gap-5 xl:gap-6 2xl:gap-7">
            ...
            <div className="bg-black/25 backdrop-blur-md border border-white/15 rounded-2xl xl:rounded-3xl p-5 xl:p-7 2xl:p-9">
              ...
              <div className="grid grid-cols-2 gap-3 xl:gap-4">
                {stats.map((s) => (
                  <div key={s.label} className="group flex flex-col items-center justify-center py-4 xl:py-5 2xl:py-6 px-3 rounded-xl xl:rounded-2xl bg-white/10 border border-white/10 hover:bg-white/15 hover:-translate-y-0.5 transition-all duration-200">
                    <span className="text-2xl xl:text-3xl 2xl:text-4xl font-black text-white tracking-tight">{s.value}</span>
                    <span className="text-white/55 text-[10px] xl:text-xs 2xl:text-sm uppercase tracking-widest mt-1 font-semibold">{s.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-black/20 backdrop-blur-md border border-white/15 rounded-2xl xl:rounded-3xl p-5 xl:p-7 2xl:p-9">
              ...
              <div className="flex flex-col gap-2.5 xl:gap-3 2xl:gap-4">
                {highlights.map((h, i) => (
                  <div key={i} className="flex items-center gap-3 xl:gap-4 px-4 py-3 xl:px-5 xl:py-3.5 2xl:px-6 2xl:py-4 rounded-xl xl:rounded-2xl bg-white/10 border border-white/10 hover:bg-white/15 hover:translate-x-1 transition-all duration-200">
                    <span className="text-lg xl:text-xl 2xl:text-2xl">{h.icon}</span>
                    <span className="text-white font-semibold text-sm xl:text-base 2xl:text-lg">{h.text}</span>
                  </div>
                ))}
              </div>
            </div>
