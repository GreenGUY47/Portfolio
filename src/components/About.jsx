...
import {
  faReact, faHtml5, faCss3Alt, faJs, faNodeJs, faNpm, faGithub, faFigma,
} from "@fortawesome/free-brands-svg-icons";
import {
  faDatabase, faCode, faBriefcase, faGraduationCap, faRocket,
  faLayerGroup, faPalette, faServer, faBolt,
...
const serviceIcons = [
  ...
  { icon: faServer,     color: "text-emerald-300", bg: "bg-emerald-400/10 border-emerald-400/20" },
...
const AboutMe = () => {
  ...
  const timeline = t('about.journey.items', { returnObjects: true }).map((item, i) => ({
    ...item,
    ...timelineIcons[i],
  }));

  const services = t('about.services.items', { returnObjects: true }).map((item, i) => ({
    ...item,
    ...serviceIcons[i],
  }));
  ...
  return (
    <section className="relative w-full min-h-screen bg-orange-500 text-white overflow-x-hidden font-sans selection:bg-white/30 dark:bg-slate-900 dark:bg-gradient-to-br dark:from-slate-900 dark:to-slate-800">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-[0.035]" style={{ backgroundImage: "radial-gradient(circle, white 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
      ...
      <div className="relative z-10 max-w-screen-xl 2xl:max-w-screen-2xl mx-auto px-5 sm:px-8 lg:px-14 xl:px-20 2xl:px-28 py-14 xl:py-20 2xl:py-28">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12 xl:mb-16 2xl:mb-20">
          <div>
            ...
            <h1 className="font-black cursor-default text-white leading-none tracking-tighter" style={{ fontSize: "clamp(2.8rem, 7vw, 7rem)" }}>
              {t('about.heading')}
            </h1>
          </div>
            ...
            <div className="relative flex flex-col gap-0">
              ...
              {timeline.map((item, i) => (
                <div key={i} className="relative flex gap-4 xl:gap-5 pb-7 xl:pb-9 last:pb-0">
                  <div className={`shrink-0 w-10 h-10 xl:w-12 xl:h-12 rounded-full ${item.accent} flex items-center justify-center z-10 dark:bg-slate-700/50`}>
                    <FontAwesomeIcon icon={item.icon} className="text-orange-600 text-sm xl:text-base dark:text-white/90" />
                  </div>
                  <div className="pt-1 xl:pt-1.5">
                    <p className="text-white/50 text-[10px] xl:text-xs 2xl:text-sm font-semibold uppercase tracking-widest mb-1">{item.year}</p>
                    <h3 className="text-white font-bold text-sm xl:text-base 2xl:text-lg mb-1.5">{item.role}</h3>
                    <p className="text-white/70 text-xs xl:text-sm 2xl:text-base leading-relaxed">{item.detail}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mb-12 xl:mb-16 2xl:mb-20">
          ...
          <div className="grid grid-cols-1 cursor-pointer sm:grid-cols-2 xl:grid-cols-4 gap-4 xl:gap-5 2xl:gap-6">
            {services.map((s, i) => (
              <div key={i} className={`group flex flex-col gap-3 xl:gap-4 p-5 xl:p-6 2xl:p-7 rounded-2xl xl:rounded-3xl border backdrop-blur-md bg-black/20 hover:-translate-y-1 hover:bg-black/30 transition-all duration-300 ${s.bg} dark:hover:bg-slate-800/50 dark:border-slate-400`}>
                ...
                <h3 className="text-white font-bold text-sm xl:text-base 2xl:text-lg leading-tight">{s.title}</h3>
                <p className="text-white/65 text-xs xl:text-sm 2xl:text-base leading-relaxed">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
